...
/// may be removed. This program implements its own garbage collection logic which ensures we do
...
async fn main() -> Result<(), ()> {
    ...
    let new_safepoint = match worker::get_new_safepoint(options.clone()) {
        Some(sp) => sp,
        None => return Ok(()),
    };
    ...
    let successful = txn_client
        ...
        .await
        .expect("failed to submit safepoint to tikv pd");
