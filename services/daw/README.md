...
TiKV, with helpers that understand the [timbre](../../crates/timbre) key
...
`REDIS_URL` points at the same Redis (and DB index) the indexer uses for its
timestamp entries, e.g. `redis://127.0.0.1:6379`.
...
`DATAPLANE_ID` and `INSTANCE_ID` are the `u8` identifiers from the polyphony
config.
