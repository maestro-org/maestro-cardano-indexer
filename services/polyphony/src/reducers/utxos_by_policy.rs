/*
   ...
   Creates reducer outputs to signal that a UTxO containing at least one of an
   asset of the policy was consumed or produced, along with the amounts of the
   assets of the policy in the UTxO and the address which controls the UTxO.
*/
...
pub struct Output {
    ...
    pub u_hash: [u8; 32],
    pub u_index: u64,
    /// address and assets in the policy
    pub action: UtxoAction<(Address, KeyValuePairs<Bytes, u64>)>,
}
...
impl Reducer {
    fn process_consumed_txo(
        ...
        if let Some(ma) = multiassets {
            for (policy, _assets) in ma.iter() {
                outputs.push(ReducerOutput::UtxosByPolicy(Output {
                    policy: *policy,
                    slot,
                    u_hash: **input.hash(),
                    u_index: input.index(),
        ...
        if let Some(ma) = multiassets {
            for (policy, assets) in ma.iter() {
                outputs.push(ReducerOutput::UtxosByPolicy(Output {
                    ...
                    action: UtxoAction::Produced((address.clone(), assets.clone())), // TODO encode in place?
                }))
            }
        }
...
impl Config {
    pub fn plugin(self, policy: &crosscut::policies::RuntimePolicy) -> super::Reducer {
        ...
        super::Reducer::UtxosByPolicy(reducer)
    }
}
