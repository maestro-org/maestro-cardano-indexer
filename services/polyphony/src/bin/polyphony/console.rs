...
use log::Log;
use polyphony::bootstrap::Pipeline;
    ...
    fn refresh(&self, pipeline: &Pipeline) {
        for tether in pipeline.tethers.iter() {
            ...
            match tether.read_metrics() {
                ...
                Err(err) => {
                    tracing::warn!("couldn't read metrics: {err:?}");
                }
            }
        }
    }
        ...
        for tether in pipeline.tethers.iter() {
            match tether.check_state() {
                gasket::runtime::TetherState::Dropped => {
                    tracing::error!("[{}] stage tether has been dropped", tether.name());
                }
                ...
                gasket::runtime::TetherState::Alive(state) => {
                    tracing::debug!("[{}] stage is alive with state: {:?}", tether.name(), state);
                    match tether.read_metrics() {
                        Ok(readings) => {
                            for (key, value) in readings {
                                tracing::debug!(
                        ...
                        Err(err) => {
                            tracing::error!("[{}] error reading metrics: {}", tether.name(), err)
                        }
                    }
                }
            }
