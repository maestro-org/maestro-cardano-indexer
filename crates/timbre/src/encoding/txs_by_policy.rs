...
use crate::encoding::encode::{encode_short_bytestring, BREAK};

use super::{
    decode::decode_short_bytestring,
    encode::{KeyEncoder, PREFIX_DATA, REDUCER_TXS_BY_POLICY},
    RangeBound, Slot, TimbreError,
};

use base64::{engine::general_purpose as b64, Engine};

// <DATAPLANE><INSTANCE><DATA><TXS_BY_POLICY_TAG><policy><BREAK><u64(slot)><u16(block index)>
pub fn encode_txs_by_policy_key(
    ...
    key.push(REDUCER_TXS_BY_POLICY);
...
pub struct TxsByPolicyKey {
    pub policy: [u8; 28],
    pub slot: u64,
    pub blk_index: u16,
}

pub fn decode_txs_by_policy_key(bytes: &[u8]) -> TxsByPolicyKey {
    let expected_len = 4 + 28 + 1 + 8 + 2;
    assert_eq!(bytes[3], REDUCER_TXS_BY_POLICY);
...
#[derive(Clone, Debug)]
pub struct TxsByPolicyCursor {
    pub slot: u64,
    pub blk_index: u16,
}

impl Slot for TxsByPolicyCursor {
    fn slot(&self) -> u64 {
        self.slot
    }
}
...
pub fn encode_txs_by_policy_cursor(slot: u64, blk_index: u16) -> String {
...
pub fn decode_txs_by_policy_cursor(b64_cursor: &String) -> Result<TxsByPolicyCursor, TimbreError> {
    ...
    Ok(TxsByPolicyCursor { slot, blk_index })
}

// --- value

// <txhash (32b)><sbs(assetname1)><sbs(assetname2)>...
pub fn encode_txs_by_policy_value(tx_hash: [u8; 32], assets: Vec<Vec<u8>>) -> Vec<u8> {
    let expected_len = 32 + assets.iter().map(|x| 1 + x.len()).sum::<usize>();
    let mut key = Vec::with_capacity(expected_len);
...
pub fn decode_txs_by_policy_value(bytes: &[u8]) -> ([u8; 32], Vec<Vec<u8>>) {
    let mut c = 0; // cursor
